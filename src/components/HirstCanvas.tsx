...
const HirstCanvas = (props) => {

  let text = props.text
  let name = props.name
  let hash

  const colours = ["#63b598",	"#ce7d78",	"#ea9e70",	"#a48a9e",	"#c6e1e8",	"#648177",	"#0d5ac1",
  ...
  function genHash() {
    hash = crypto.createHmac('sha256', name)
                 .update(text)
                 .digest('hex');
  }
  ...
  useEffect(() => {
    ...
    if (text.trim() != "" && name.trim() != "") {
      ...
      genHash()
      let t = ctx.measureText(hash)
      console.log(`text width: ${t.width}`)
      
      ctx.fillText(hash, 100, 225)
      let pos = 1
  ...
  return (
    <div>
      <canvas id='myPaintingCanvas' width='225px' height='225px' style={{ border: '1px solid'}}></canvas>
    </div>
  )
}

export default HirstCanvas